...
function LabThree() {
  ...
  const laboratories=useSelector((state)=>state.adminReducers.labs)
  const labs=[]
  if(laboratories && laboratories.length>3){
    for(let i=0;i<3;i++){
        labs[i]=laboratories[i]
    }
  }
  console.log(labs)
    return (
        <>
            ...
            {labs && labs.map((lab,i)=>{
                ...
                <div key={i} className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 ">
                    <a href="#">
                    <img className="rounded-t-lg" src={lab.images[0]} alt="" />
                    </a>
                    <div className="p-5">
                    <a href="#">
